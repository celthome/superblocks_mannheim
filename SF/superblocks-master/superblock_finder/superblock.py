...
postgis_connection = create_engine("postgresql://postgres:postgres@localhost:5432/superblocks")

# Funktion zur Überprüfung der Verbindung
def check_connection(engine):
    try:
        # Test-Abfrage (einfacher SQL-Befehl)
...
check_connection(postgis_connection)
...
postgis_connection = create_engine(f"postgresql://{os.getenv('POSTGRES_USER', 'postgres')}:"
                                   ...
                                   f"@{os.getenv('POSTGRES_HOST', 'localhost')}:5432/{os.getenv('POSTGRES_DATABASE', 'superblocks')}")


# In[6]:


def find_superblocks(job_id: int):
